...
*/
/******************************************************************************/
Mtx44& Mtx44::SetToIdentity(void) {
	SetToZero();
	a[0] = a[5] = a[10] = a[15] = 1;
...
{
	Vector3 result;
	result.x = rhs.x * a[0] + rhs.y * a[4] + rhs.z * a[8] + a[12];
	result.y = rhs.x * a[1] + rhs.y * a[5] + rhs.z * a[9] + a[13];
	result.z = rhs.x * a[2] + rhs.y * a[6] + rhs.z * a[10] + a[14];
	return result;
}
...
*/
/******************************************************************************/
Mtx44& Mtx44::SetToRotation(float degrees, float axisX, float axisY, float axisZ) throw( DivideByZero ) {
